...
# from Schedule_Maker.df_to_schedule import cost_funs
...
def make_slot_and_flight(slot_time: float, slot_index: int,
                         ...
                         jump_2: float = None, empty_slot=False, fl_type: str = None, missed_connecting=None,
                         curfew=None):
...
def slot_flight_list_from_Schedule_package(sl_list, fl_list):
    slot_list, flight_list = [], []

    for slot in sl_list:
        slot_list.append(Slot(slot['index'], slot['new_time']))

    for i, flight in enumerate(fl_list):
        flight_list.append(Flight(slot=slot_list[i], flight_name=flight['flight_name'],
                                  airline_name=flight['airline_name'], eta=flight['eta'],
                                  delay_cost_vect=flight['delay_cost_vect'], cost_fun=flight['cost_fun'],
                                  fl_type=flight['fl_type'],
                                  missed_connecting=flight['missed_connecting'], curfew=flight['curfew']))
